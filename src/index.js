...
export class UCloudUFile {
  ...
  getContentMd5(file, success) {
    ...
    function loadNext() {
      ...
      let end =
        start + that.sliceSize >= file.size
          ? file.size
  ...
  initMultipartUpload(success, error, progress, file, fileName) {
    ...
    this.getUFileToken(
      ...
      function(token) {
        let ajax = that.createAjax();
        let url =
          that.getBucketUrl() + encodeURIComponent(fileName) + "?uploads";
        ajax.open(method, url, true);
        ajax.setRequestHeader("Authorization", token);
  ...
  hitSliceUpload(file, success, error, progress) {
    ...
    let suffix = "";
    if (
      Object.prototype.toString.call(match) === "[object Array]" &&
      match.length > 2
    ) {
      name = match[1];
      suffix = match[2];
    ...
    const fileRename = `${suffix}/${unique}/${fullName}`;
